...
OLD="artifacts/fly_router_v0_3_2.pt"
OUT="artifacts/fly_router_v0_3_3.pt"
...
if [[ -f "$OLD" ]]; then
  echo
  echo "=== v0.3.2 gratitude baseline ==="
...
echo
echo "=== training v0.3.3: syllable-aware gratitude patch ==="
...
echo
echo "=== v0.3.3 brain integrity check ==="
python training/diagnose_brain.py \
  --model "$OUT" \
  --scaffold "$SCAFFOLD"

echo
echo "=== v0.3.3 gratitude regression ==="
python training/eval_router.py \
  --model "$OUT" \
  --dataset "$GRATITUDE"

echo
echo "=== v0.3.3 full held-out regression ==="
python training/eval_router.py \
  --model "$OUT" \
  --dataset "$FULL"
